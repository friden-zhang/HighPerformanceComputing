...
// (oh, ow). Each thread computes K_TILE output channels.
__global__ void conv3d_mdspan_shared_input_tile_kblock_nk_od(
    ...
    int KD, int KH, int KW, int K_TILE) {
...
cudaError_t launch_conv3d_mdspan_shared_input_tile_kblock_nk_od(
    const float *__restrict__ input, const float *__restrict__ kernel,
    float *__restrict__ output, int N, int C, int D, int H, int W, int K,
  ...
  if (stream) {
    conv3d_mdspan_shared_input_tile_kblock_nk_od<<<grid, block, shared_bytes,
                                                   stream>>>(
        input, kernel, output, N, C, D, H, W, K, KD, KH, KW, K_TILE);
  } else {
    conv3d_mdspan_shared_input_tile_kblock_nk_od<<<grid, block, shared_bytes>>>(
        input, kernel, output, N, C, D, H, W, K, KD, KH, KW, K_TILE);
