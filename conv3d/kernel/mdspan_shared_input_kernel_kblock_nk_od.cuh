...
// Conv3D: mdspan indexing, block fixed (n, k-block, od), shared input + shared
// kernel weights for (oh, ow). Each thread computes K_TILE output channels.
template <int BLOCK_X, int BLOCK_Y, int K_TILE>
__global__ void conv3d_mdspan_shared_input_kernel_kblock_nk_od(
  ...
  const int input_elems = C * KD * sh_plane;
  const int kernel_elems = K_TILE * C * KD * KH * KW;

  extern __shared__ float shmem[];
  float *sh_input = shmem;
  float *sh_kernel = shmem + input_elems;
  ...
  for (int idx = tid; idx < input_elems; idx += num_threads) {
    int tmp = idx;
  ...
  for (int idx = tid; idx < kernel_elems; idx += num_threads) {
    int tmp = idx;
    const int kw = tmp % KW;
    tmp /= KW;
    const int kh = tmp % KH;
    ...
    const int k = k_block * K_TILE + kt;
    float val = 0.0f;
    if (k < K) {
      val = mdspan_kernel(k, c, kd, kh, kw);
    }
    sh_kernel[idx] = val;
  }
  ...
  const int kernel_stride = C * KD * KH * KW;
  const int k_base = k_block * K_TILE;

  for (int kt = 0; kt < K_TILE; ++kt) {
    const int k = k_base + kt;
    if (k >= K)
      continue;

    const float *sh_kernel_k = sh_kernel + kt * kernel_stride;

    float sum = 0.0f;
    for (int c = 0; c < C; ++c) {
      for (int kd = 0; kd < KD; ++kd) {
        const int base = (c * KD + kd) * sh_plane;
        const float *sh_kernel_ckd =
            sh_kernel_k + (c * KD + kd) * KH * KW;
        for (int kh = 0; kh < KH; ++kh) {
          const int sh_y = threadIdx.y + kh;
          const int row = base + sh_y * sh_w;
          const float *sh_kernel_row = sh_kernel_ckd + kh * KW;
          for (int kw = 0; kw < KW; ++kw) {
            const int sh_x = threadIdx.x + kw;
            sum += sh_input[row + sh_x] * sh_kernel_row[kw];
          }
        }
      }
    }
...
template <int BLOCK_X, int BLOCK_Y, int K_TILE>
cudaError_t launch_conv3d_mdspan_shared_input_kernel_kblock_nk_od(
    const float *__restrict__ input, const float *__restrict__ kernel,
    float *__restrict__ output, int N, int C, int D, int H, int W, int K,
  ...
  const size_t shared_bytes =
      ...
      static_cast<size_t>(K_TILE) * C * KD * KH * KW * sizeof(float);
  ...
  if (stream) {
    conv3d_mdspan_shared_input_kernel_kblock_nk_od<BLOCK_X, BLOCK_Y, K_TILE>
